...
@ManagedBean(name = "empresaForm2")
@SessionScoped
public class MantenimientoEmpresaEmpleadoProductoFormAction extends
	...
	private Log log = LogFactory.getLog(MantenimientoEmpresaEmpleadoProductoFormAction.class);
	...
	public String Eliminarempresa() throws Exception {
		...
		try {
			if (log.isInfoEnabled()) {
				log.info("Entering my method Eliminar EMPRESA 'updateDESHABILITAR()'" + getIde());
			}
...
//			 
	  ...
	  if (verificarEmpleadoConEmpresa(parametroObtenido) == false) {
		  ...
		  context.execute(Constants.MANT_EMPRESA_EMPLEADO_PRODUCTO_FORM_LIST_PAGE);
	}
...
//	FaceMessage.FaceMessageInfo("ALERTA", "La Empresa no se puede elminar ya que se encontraron productos con esta empresa");
	...
	public MantenimientoEmpresaEmpleadoProductoFormAction() {
		init();
	}
